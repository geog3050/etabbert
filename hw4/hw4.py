...
def calculateDistanceFromPointsToPolylines(input_geodatabase, fcPoint, fcPolyline):
    ...
    try:
        ...
        print("Analysis complete! Your distance measurement has been appended to the file called '" + fcPoint + "'")
    except:
        print("An exception occurred while analyzing the distance between " + fcPoint + " and " + fcPolyline)
...
def countPointsByTypeWithinPolygon(input_geodatabase, fcPoint, pointFieldName, pointFieldValue, fcPolygon):
    ...
    delimField = arcpy.AddFieldDelimiters(fcPoint, pointFieldName)
    cursor = arcpy.da.SearchCursor(fcPoint, ["NAME"], delimField + "LIKE '" + pointFieldValue + "'")

    for row in cursor:
        arcpy.SummarizeWithin_analysis(fcPolygon, row, "summarizeOutput")

    #I'm having some issues understanding this problem and the next one. I think I'm on the right track but I'm confused on how to use the summarize within
    #with the search cursor to only look at specific points.
######################################################################
# Problem 3 (50 points)
# 
