    ...
    for row in file:
        ...
        for element in row.strip("\n"):
            if element.isalpha() == True or element.isdigit() == True:
                ...
                pokemon.append(placeholder)
                placeholder = ""
        pokemon.append(placeholder)
        participants.append(pokemon)

    for i in range(0, len(participants)):
        participants[i][2] = float(participants[i][2])
        participants[i][3] = float(participants[i][3])
...
def tournament(participants):
    ...
    for p1 in range(0, len(participants)):
